...
//                        }
                            if (musicTwoRowItemRenderer.isSong) {
                                ...
                                val artists = ytItem?.artists?.map {
                                    Artist(
                                        name = it.name,
                                        id = it.id
                                    )
                            ...
                            if (ytItem != null) {
                                val content = Content(
                                    ...
                                    playlistId = null,
                                    browseId = null,
                                    thumbnails = result1.musicResponsiveListItemRenderer!!.thumbnail?.musicThumbnailRenderer?.thumbnail?.thumbnails?.toListThumbnail()
                                        ?: listOf(),
                                    title = ytItem.title,
                                    videoId = ytItem.id,
                                    views = "",
...
fun parseSongFlat(data: MusicResponsiveListItemRenderer?, context: Context): Content? {
    if (data?.flexColumns != null) {
        ...
        return Content(
            ...
            playlistId = null,
            browseId = null,
            thumbnails = data.thumbnail?.musicThumbnailRenderer?.thumbnail?.thumbnails?.toListThumbnail()
                ?: listOf(),
...
fun parsePlaylist(data: MusicTwoRowItemRenderer, context: Context): Content {
...
fun parseSongArtistsRuns(runs: List<Run>, context: Context): List<Artist> {
...
fun parseRelatedArtists(data: MusicTwoRowItemRenderer): Content {
    return Content(
        ...
        thumbnails = data.thumbnailRenderer.musicThumbnailRenderer?.thumbnail?.thumbnails?.toListThumbnail()
            ?: listOf(),
        title = data.title.runs?.get(0)?.text ?: "",
        videoId = null,
        views = null
...
fun Thumbnail.toThumbnail(): com.universe.audioflare.data.model.searchResult.songs.Thumbnail {
    return com.universe.audioflare.data.model.searchResult.songs.Thumbnail(
...
fun List<Thumbnail>.toListThumbnail(): List<com.universe.audioflare.data.model.searchResult.songs.Thumbnail> {
    val list = mutableListOf<com.universe.audioflare.data.model.searchResult.songs.Thumbnail>()
    this.forEach {
        ...
        HomeItem(
            ...
            contents = explore.released.map {
                Content(
                    ...
                    browseId = it.id,
                    thumbnails = listOf(
                        com.universe.audioflare.data.model.searchResult.songs.Thumbnail(
                            522,
                            it.thumbnail,
                            522
                        )
