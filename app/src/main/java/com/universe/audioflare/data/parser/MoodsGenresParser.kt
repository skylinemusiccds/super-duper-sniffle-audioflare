...
import com.universe.audioflare.data.model.explore.mood.genre.ItemsSong
import com.universe.audioflare.data.model.explore.mood.genre.Title
import com.universe.audioflare.data.model.explore.mood.moodmoments.Content
...
fun parseMoodsMomentObject(data: BrowseResponse?): MoodsMomentObject? {
    if (data != null){
        val title = data.header?.musicHeaderRenderer?.title?.runs?.get(0)?.text ?: ""
        val items = data.contents?.singleColumnBrowseResultsRenderer?.tabs?.get(0)?.tabRenderer?.content?.sectionListRenderer?.contents
                    ...
                    if (!contents.isNullOrEmpty()) {
                        for (content in contents) {
                            if (content.musicResponsiveListItemRenderer != null) {
                                ...
                                //Playlist
                                val thumbnails =
                                    content.musicTwoRowItemRenderer?.thumbnailRenderer?.musicThumbnailRenderer?.thumbnail?.thumbnails?.toListThumbnail()
                                var subtitle = ""
        ...
        if (items != null) {
            for (item in items) {
                if (item.musicCarouselShelfRenderer != null) {
                    ...
                    val listContent: MutableList<com.universe.audioflare.data.model.explore.mood.genre.Content> = mutableListOf()
                    if (!contents.isNullOrEmpty()) {
                        for (content in contents) {
                            if (content.musicResponsiveListItemRenderer != null) {
                                //Song
                                ...
                                val playlistBrowseId =
                                    content.musicTwoRowItemRenderer?.navigationEndpoint?.browseEndpoint?.browseId
                                listContent.add(
                                    com.universe.audioflare.data.model.explore.mood.genre.Content(
                                        playlistBrowseId = playlistBrowseId ?: "",
